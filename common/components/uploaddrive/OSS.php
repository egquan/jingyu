...
class OSS extends DriveInterface
{
    ...
    public function config($maxSize, $path = '', $expire = 30, $type = Attachment::UPLOAD_TYPE_FILES, $callbackUrl = '')
    {
        ...
        if (!empty($config['storage_aliyun_user_url'])) {
            $host = $config['storage_aliyun_transport'] . "://" . $config['storage_aliyun_user_url'];
        }
    ...
    protected function baseUrl($baseInfo, $fullPath)
    {
        ...
        if (!empty($user_url)) {
            $baseInfo['url'] = $this->config['storage_aliyun_transport'] . '://' . $user_url . '/' . $baseInfo['url'];
        } else {
    ...
    protected function create()
    {
        $this->adapter = new OssAdapter([
            ...
            'endpoint' => $this->config['storage_aliyun_endpoint'],
            // 'timeout' => 3600,
            // 'connectTimeout' => 10,
            // 'isCName' => false,
            // 'token' => '',
