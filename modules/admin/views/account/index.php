...
?>
    ...
    <script>
        ...
        layui.use(['form', 'table'], function () {
            ...
            table.render({
                elem: '#currentTableId',
                url: '<?=Url::to(['/admin/account/index'])?>',
                toolbar: '#toolbarDemo',
                ...
                cols: [[
                    ...
                    {field: 'email', width: 200, title: 'Email'},
                    {
                        field: 'status', width: 80, title: '状态', templet: function (d) {
                            ...
                            return '<span style="color: #cc0a0d;">禁用</span>';
                        }
                    },
            ...
            table.on('toolbar(currentTableFilter)', function (obj) {
                if (obj.event === 'add') {  // 监听添加操作
                    var index = layer.open({
                        ...
                        shadeClose: true,
                        area: ['100%', '100%'],
                        content: '<?=Url::to(['/admin/account/create'])?>',
                    });
                    ...
                    if (data.length <= 0) {
                        layer.msg('请选择要删除的用户！', {icon: 3, time: 2000, offset: 't'});
                        return false;
                    }
                    for (i = 0, len = data.length; i < len; i++) {
                        $.ajax({
                            url: '<?=Url::to(['/admin/account/delete'])?>?id=' + data[i].id,
                            type: 'post',
                            dataType: 'json',
                            timeout: 2000
                        });
            ...
            table.on('tool(currentTableFilter)', function (obj) {
                ...
                if (obj.event === 'edit') {
                    ...
                    var index = layer.open({
                        ...
                        shadeClose: true,
                        area: ['100%', '100%'],
                        content: '<?=Url::to(['/admin/account/update'])?>?id=' + data.id,
                    });
                    $(window).on("resize", function () {
                        layer.full(index);
                    });
                    ...
                    layer.confirm('真的删除行么', function (index) {
                        layer.close(index);
                        $.ajax({
                            url: '<?=Url::to(['/admin/account/delete'])?>?id=' + obj.data.id,
                            type: 'post',
                            dataType: 'json',
                            timeout: 2000,
